...
const REGLES_RGIE: Record<TypePiece, RegleRgie> = {
  SALON: {
    ...
    notes: ["Min 5 prises (Art. 6.6)", "1 prise tous les 5 m²", "1 luminaire / 6 m² conseillé"],
  },
  SEJOUR: {
  ...
  CHAMBRE: {
    ...
    circuitsDedies: [],
    diffObligatoire: false,
    notes: ["Min 3 prises (Art. 6.6)", "1 prise tous les 8 m²", "1 luminaire / 8 m²"],
  },
  CUISINE: {
    label: "Cuisine",
    prisesMin: 4,
    ...
    circuitsDedies: ["Four (6 mm², 32A)", "Plaque (6 mm², 32A)", "Lave-vaisselle (2.5 mm², 16A)", "Réfrigérateur (2.5 mm², 16A)", "Micro-ondes (2.5 mm², 16A)"],
    diffObligatoire: false,
    notes: [
  ...
  WC: {
    ...
    circuitsDedies: [],
    diffObligatoire: false,
    notes: ["1 point d'éclairage minimum", "Prise possible si hors zone humide"],
  },
  BUREAU: {
    label: "Bureau",
    prisesMin: 3,
    ...
    circuitsDedies: [],
    diffObligatoire: false,
    notes: ["Min 3 prises", "1 luminaire / 5 m²", "1 circuit informatique recommandé (Art. 6.6)"],
  },
  GARAGE: {
    label: "Garage",
    prisesMin: 2,
  ...
  COULOIR: {
    ...
    circuitsDedies: [],
    diffObligatoire: false,
    notes: ["Min 1 prise", "1 luminaire / 4 m²", "Télérupteur recommandé pour éclairage multi-points"],
  },
  AUTRE: {
    label: "Autre",
    prisesMin: 1,
...
// ----------------------------------------------------------------
const SECTIONS = {
  "1.5": { label: "1.5 mm² — Éclairage (10A)", color: "bg-yellow-100 text-yellow-800" },
  "2.5": { label: "2.5 mm² — Prises (16A)", color: "bg-blue-100 text-blue-800" },
  "6":   { label: "6 mm² — Gros appareils (32A)", color: "bg-orange-100 text-orange-800" },
  "10":  { label: "10 mm² — Tableau / général", color: "bg-red-100 text-red-800" },
};
...
function calculerPiece(p: Piece): ResultatPiece {
  ...
  // ── Nombre de prises ────────────────────────────────────────────
  const prisesParSurface = regle.prisesParM2 > 0 ? Math.ceil(surface / regle.prisesParM2) : regle.prisesMin;
  const nbPrisesCalc = p.nbPrisesManuel !== null
    ? p.nbPrisesManuel
    : Math.max(regle.prisesMin, prisesParSurface);

  // ── Nombre de luminaires ─────────────────────────────────────────
  // Ex : 3 lampes dans 5 m → espacement = 5/(3+1) = 1.25 m
  const nbLampesAuto = regle.m2ParLampe > 0
    ? Math.max(regle.pointsLumMin, Math.ceil(surface / regle.m2ParLampe))
    : regle.pointsLumMin;
  const nbPointsLumCalc = p.nbPointsLumManuel !== null ? p.nbPointsLumManuel : nbLampesAuto;

  // Espacement entre luminaires (sur la longueur)
  const spacing = nbPointsLumCalc > 0
  ...
  const totalBrut15 = p.distTableau + cablePlafond + chutesLuminaires + monteeInterrupteur;
  ...
  const cable25 = Math.ceil(
    (p.distTableau + perimetre + nbPrisesCalc * STUB_PRISE) * nbCircuitsPrises * FACTEUR_POSE
  );
  ...
  if (nbPrisesCalc < regle.prisesMin) alertes.push(`⚠ Nombre de prises inférieur au minimum RGIE (${regle.prisesMin})`);
  if (regle.diffObligatoire) alertes.push("⚡ Différentiel 30 mA obligatoire dans cette pièce");
  if (p.type === "CUISINE" && !p.appareils.some((a) => a.includes("Four")))
    alertes.push("⚠ Circuit dédié four non prévu (obligatoire RGIE Art. 6.6.3)");
  if (p.type === "CUISINE" && !p.appareils.some((a) => a.includes("Plaque")))
    alertes.push("⚠ Circuit dédié plaque non prévu (obligatoire RGIE Art. 6.6.3)");
...
export default function EstimateurPage() {
  ...
  const resultats: ResultatPiece[] = useMemo(() => pieces.map(calculerPiece), [pieces]);
  ...
  const totaux = useMemo(() => {
    ...
    const alertes = resultats.flatMap((r) => r.alertes.map((a) => `[${REGLES_RGIE[r.type].label}] ${a}`));
    return { cable15, cable25, cable6, prises, points, circEcl, circPr, circDed, alertes };
  }, [resultats]);
  ...
  const updatePiece = <K extends keyof Piece>(id: string, key: K, value: Piece[K]) =>
  ...
  const toggleAppareil = (id: string, app: string) =>
    setPieces((prev) =>
      prev.map((p) =>
        p.id !== id
          ? p
          : { ...p, appareils: p.appareils.includes(app) ? p.appareils.filter((a) => a !== app) : [...p.appareils, app] }
      )
    );
  ...
  const changePieceType = (id: string, newType: TypePiece) =>
    setPieces((prev) =>
      prev.map((p) =>
        p.id !== id ? p : { ...p, type: newType, nbPrisesManuel: null, nbPointsLumManuel: null, appareils: [] }
      )
    );

  // Stepper générique pour prises / luminaires
  const StepperField = ({
    label, value, isAuto, rgieBadge, onIncrement, onDecrement, onReset, warn,
  }: {
    label: string; value: number; isAuto: boolean;
    rgieBadge: string; onIncrement: () => void; onDecrement: () => void;
    onReset: () => void; warn?: string;
  }) => (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center gap-1.5">
        <span className="text-xs font-semibold text-[color:var(--ink)]">{label}</span>
        {isAuto ? (
          <span className="rounded-full bg-green-100 px-2 py-0.5 text-[10px] font-bold text-green-700">RGIE auto</span>
        ) : (
          <button
            onClick={onReset}
        ...
        {warn && <span className="ml-auto text-[10px] font-bold text-red-500">{warn}</span>}
      </div>
      <div className="flex items-center gap-2">
        ...
        >−</button>
        <div className={`flex h-10 min-w-[3.5rem] items-center justify-center rounded-xl border-2 px-3 text-xl font-bold transition ${
          !isAuto
      ...
      <div className="mx-auto w-full max-w-6xl space-y-6">
        ...
        <div className="fade-up">
          ...
          <p className="mt-1 text-sm text-[color:var(--muted)]">
            Saisir les pièces, dimensions et équipements. Les quantités de câble et matériel sont calculées selon les règles RGIE.
          </p>
        </div>

        {/* Config globale */}
        <div className="fade-up rounded-2xl border border-black/10 bg-[color:var(--surface)] p-5">
          <p className="mb-3 text-sm font-semibold text-[color:var(--ink)]">Paramètres généraux</p>
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <div className="flex flex-col justify-end">
              <p className="mb-1 text-xs text-[color:var(--muted)]">Facteur de pose appliqué</p>
            ...
            <div className="flex flex-col justify-end">
              <p className="mb-1 text-xs text-[color:var(--muted)]">Max prises par circuit (RGIE)</p>
              <div className="flex h-10 items-center rounded-xl border border-black/10 bg-[color:var(--panel)] px-3 text-sm font-semibold text-[color:var(--ink)]">
            ...
            return (
              ...
              >
                ...
                <div className="mb-3 flex flex-wrap items-start justify-between gap-3">
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-3">
                      ...
                      <select
                        value={piece.type}
                        onChange={(e) => changePieceType(piece.id, e.target.value as TypePiece)}
                      ...
                      >
                        {(Object.keys(REGLES_RGIE) as TypePiece[]).map((t) => (
                          <option key={t} value={t}>{REGLES_RGIE[t].label}</option>
                        ))}
                      </select>
                      {regle.diffObligatoire && (
                        <span className="rounded-full bg-red-100 px-2 py-0.5 text-[10px] font-bold text-red-600">⚡ Diff. 30 mA</span>
                      )}
                    </div>
                    {/* Chips résumé live */}
                    {res && (
                      <div className="ml-11 flex flex-wrap gap-1.5">
                        ...
                        <span className="rounded-full bg-blue-50 px-2 py-0.5 text-[10px] font-semibold text-blue-700">
                          🔌 {res.nbPrisesCalc} prise{res.nbPrisesCalc !== 1 ? "s" : ""}
                        </span>
                        <span className="rounded-full bg-yellow-50 px-2 py-0.5 text-[10px] font-semibold text-yellow-700">
                          💡 {res.nbPointsLumCalc} pt{res.nbPointsLumCalc !== 1 ? "s" : ""} lum.
                        </span>
                        <span className="rounded-full bg-purple-50 px-2 py-0.5 text-[10px] font-semibold text-purple-700">
                          ⚡ {res.nbCircuitsEclairage + res.nbCircuitsPrises + res.appareils.length} circuit{res.nbCircuitsEclairage + res.nbCircuitsPrises + res.appareils.length !== 1 ? "s" : ""}
                        </span>
                ...
                <div className="mb-4 grid gap-3 sm:grid-cols-4">
                  <label className={labelCls}>
                    Longueur (m)
                    <input type="number" min={0.5} step={0.1} value={piece.longueur}
                      onChange={(e) => updatePiece(piece.id, "longueur", Number(e.target.value))}
                      className={inputCls} />
                  </label>
                  <label className={labelCls}>
                    Largeur (m)
                    <input type="number" min={0.5} step={0.1} value={piece.largeur}
                      onChange={(e) => updatePiece(piece.id, "largeur", Number(e.target.value))}
                      className={inputCls} />
                  </label>
                  <label className={labelCls}>
                    Hauteur (m)
                    <input type="number" min={1} step={0.1} value={piece.hauteur}
                      onChange={(e) => updatePiece(piece.id, "hauteur", Number(e.target.value))}
                      className={inputCls} />
                  </label>
                  <label className={labelCls}>
                    Distance tableau (m)
                    <input type="number" min={1} step={0.5} value={piece.distTableau}
                      onChange={(e) => updatePiece(piece.id, "distTableau", Number(e.target.value))}
                      className={inputCls} />
                  </label>
                </div>
                ...
                <div className="mb-4 grid gap-4 rounded-2xl border border-black/10 bg-[color:var(--panel)] p-4 sm:grid-cols-2">
                  <StepperField
                    ...
                    warn={res && res.nbPrisesCalc < regle.prisesMin ? `⚠ min RGIE: ${regle.prisesMin}` : undefined}
                    onIncrement={() => updatePiece(piece.id, "nbPrisesManuel", (res?.nbPrisesCalc ?? regle.prisesMin) + 1)}
                    onDecrement={() => updatePiece(piece.id, "nbPrisesManuel", Math.max(0, (res?.nbPrisesCalc ?? regle.prisesMin) - 1))}
                    onReset={() => updatePiece(piece.id, "nbPrisesManuel", null)}
                  />
                  <StepperField
                    ...
                    warn={res && res.nbPointsLumCalc < regle.pointsLumMin ? `⚠ min RGIE: ${regle.pointsLumMin}` : undefined}
                    onIncrement={() => updatePiece(piece.id, "nbPointsLumManuel", (res?.nbPointsLumCalc ?? regle.pointsLumMin) + 1)}
                    onDecrement={() => updatePiece(piece.id, "nbPointsLumManuel", Math.max(0, (res?.nbPointsLumCalc ?? regle.pointsLumMin) - 1))}
                    onReset={() => updatePiece(piece.id, "nbPointsLumManuel", null)}
                  />
                </div>
                ...
                {regle.circuitsDedies.length > 0 && (
                  <div className="mb-4">
                    <p className="mb-2 text-xs font-semibold text-[color:var(--muted)]">Circuits dédiés (cocher les appareils présents)</p>
                ...
                {res && (
                  <div className="rounded-2xl border border-black/10 bg-[color:var(--panel)] p-4">
                    <div className="mb-3 grid grid-cols-2 gap-2 text-xs text-[color:var(--muted)] sm:grid-cols-4">
                      <span>Surface : <strong className="text-[color:var(--ink)]">{res.surface.toFixed(1)} m²</strong></span>
                      <span>Prises : <strong className="text-[color:var(--ink)]">{res.nbPrisesCalc}</strong></span>
                      <span>Points lum. : <strong className="text-[color:var(--ink)]">{res.nbPointsLumCalc}</strong></span>
                      <span>Circuits : <strong className="text-[color:var(--ink)]">{res.nbCircuitsEclairage + res.nbCircuitsPrises + res.appareils.length}</strong></span>
                    </div>
                    <div className="flex flex-wrap gap-2">
                    ...
                    <details className="mt-3">
                      ...
                      <div className="mt-2 space-y-1 rounded-xl bg-yellow-50 p-3 text-xs text-yellow-900">
                        <p>
                          <strong>Luminaires calculés :</strong> {res.nbPointsLumCalc} pt
                          {" "}(surface {res.surface.toFixed(1)} m² ÷ {regle.m2ParLampe} m²/pt)
                        </p>
                        <p>
                          <strong>Espacement entre luminaires :</strong>{" "}
                          {res.detailCable15.spacing.toFixed(2)} m
                          {" "}= L ({piece.longueur} m) ÷ (n+1) = {piece.longueur} / {res.nbPointsLumCalc + 1}
                        </p>
                        <p>
                          <strong>Câble en plafond :</strong>{" "}
                          {res.detailCable15.cablePlafond.toFixed(2)} m
                          {" "}= L + l = {piece.longueur} + {piece.largeur}
                        </p>
                        <p>
                          <strong>Chutes luminaires ({res.nbPointsLumCalc} × 0.5 m) :</strong>{" "}
                        ...
                        <p className="rounded-lg bg-yellow-200 px-2 py-1 font-semibold">
                          Total brut / circuit : {res.detailCable15.totalBrut.toFixed(2)} m
                          {" "}→ × {res.nbCircuitsEclairage} circuit(s) × 1.25 (chute) = <strong>{res.cable15} m</strong>
                        </p>
                      </div>
                    </details>
                    ...
                    {res.alertes.length > 0 && (
                      <div className="mt-3 space-y-1">
                        {res.alertes.map((a, i) => (
                          <p key={i} className="text-xs font-medium text-amber-700">{a}</p>
                        ))}
                      </div>
                    )}
                    {/* Notes RGIE */}
                    {regle.notes.length > 0 && (
                      <details className="mt-2">
                        ...
                        <ul className="mt-2 space-y-1 pl-3">
                          {regle.notes.map((n, i) => (
                            <li key={i} className="text-xs text-[color:var(--muted)] before:mr-1.5 before:content-['→']">{n}</li>
                          ))}
                        </ul>
                      </details>
                    )}
        ...
        {pieces.length > 0 && (
          <div className="fade-up rounded-[28px] border border-black/10 bg-[color:var(--surface)] p-8 shadow-[0_24px_70px_-50px_rgba(0,0,0,0.45)]">
            <h2 className="mb-6 text-xl font-semibold text-[color:var(--ink)]">
              Récapitulatif général — {pieces.length} pièce{pieces.length > 1 ? "s" : ""}
            </h2>
            ...
            <div className="mb-6 grid gap-3 sm:grid-cols-3">
              {(
                [
                  { key: "cable15", label: "Câble 1.5 mm²", sublabel: "Éclairage — 10A", value: totaux.cable15, color: "bg-yellow-50 border-yellow-200 text-yellow-800" },
                  { key: "cable25", label: "Câble 2.5 mm²", sublabel: "Prises — 16A", value: totaux.cable25, color: "bg-blue-50 border-blue-200 text-blue-800" },
                  { key: "cable6",  label: "Câble 6 mm²",   sublabel: "Gros appareils — 32A", value: totaux.cable6, color: "bg-orange-50 border-orange-200 text-orange-800" },
                ] as const
              ).map((c) => (
                <div key={c.key} className={`rounded-2xl border p-5 ${c.color}`}>
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] opacity-70">{c.sublabel}</p>
            ...
            <div className="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {[
                { label: "Prises", value: totaux.prises, sub: "total installation" },
                { label: "Points lumineux", value: totaux.points, sub: "total installation" },
                { label: "Circuits éclairage", value: totaux.circEcl, sub: "disj. 10A / 1.5 mm²" },
                { label: "Circuits prises", value: totaux.circPr, sub: "disj. 16A / 2.5 mm²" },
              ].map((m) => (
                <div key={m.label} className="rounded-2xl border border-black/10 bg-[color:var(--panel)] p-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[color:var(--muted)]">{m.label}</p>
                  <p className="mt-3 text-3xl font-bold text-[color:var(--ink)]">{m.value}</p>
                  <p className="mt-1 text-xs text-[color:var(--muted)]">{m.sub}</p>
                </div>
              ))}
            </div>
            ...
            {totaux.circDed > 0 && (
              <div className="mb-6 rounded-2xl border border-orange-200 bg-orange-50 p-4">
                <p className="text-sm font-semibold text-orange-800">
                  {totaux.circDed} circuit{totaux.circDed > 1 ? "s" : ""} dédié{totaux.circDed > 1 ? "s" : ""} prévu{totaux.circDed > 1 ? "s" : ""}
                </p>
                <p className="mt-1 text-xs text-orange-700">
                  {resultats.flatMap((r) => r.appareils.map((a) => `[${REGLES_RGIE[r.type].label}] ${a}`)).join(" · ")}
                </p>
              </div>
            )}
            ...
            <div className="mb-6 rounded-2xl border border-black/10 bg-[color:var(--panel)] p-5">
              <p className="mb-3 text-sm font-semibold text-[color:var(--ink)]">Tableau électrique estimé</p>
              <div className="grid gap-2 sm:grid-cols-3 text-sm">
                <span className="text-[color:var(--muted)]">Disjoncteurs 10A (éclairage) : <strong className="text-[color:var(--ink)]">{totaux.circEcl}</strong></span>
                <span className="text-[color:var(--muted)]">Disjoncteurs 16A (prises) : <strong className="text-[color:var(--ink)]">{totaux.circPr}</strong></span>
                <span className="text-[color:var(--muted)]">Circuits dédiés (16A/32A) : <strong className="text-[color:var(--ink)]">{totaux.circDed}</strong></span>
              </div>
              <div className="mt-3 grid gap-2 sm:grid-cols-3 text-sm">
                <span className="text-[color:var(--muted)]">
                  Différentiels 30mA requis :{" "}
                  <strong className="text-[color:var(--ink)]">
                    {resultats.filter((r) => r.regle.diffObligatoire).length} pièce{resultats.filter((r) => r.regle.diffObligatoire).length > 1 ? "s" : ""}
                  </strong>
                </span>
                <span className="text-[color:var(--muted)]">
            ...
            {totaux.alertes.length > 0 && (
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5">
                <p className="mb-3 text-sm font-semibold text-amber-800">
                  ⚠ {totaux.alertes.length} point{totaux.alertes.length > 1 ? "s" : ""} RGIE à vérifier
                </p>
                <ul className="space-y-1">
                  {totaux.alertes.map((a, i) => (
                    <li key={i} className="text-xs text-amber-700 before:mr-2 before:content-['•']">{a}</li>
                  ))}
                </ul>
              </div>
            )}
            ...
            <p className="mt-6 text-center text-xs text-[color:var(--muted)]">
              Estimations basées sur les règles RGIE (éd. 2024) · Facteur de pose ×{FACTEUR_POSE} · À affiner avec métrés réels sur plan.
            </p>
          </div>
        )}
      </div>
